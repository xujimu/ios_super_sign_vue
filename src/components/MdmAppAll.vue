...
<!--      </el-table-column>-->
        ...
        width="30%">
            <span slot="footer" class="dialog-footer">
                <el-upload
                    ...
                    drag
                    :action="this.$http.defaults.baseURL + '/mdmdistribute/uploadApk'"
                    name="apk"
                    accept=".apk"
                    :multiple="false">
        ...
        width="30%">
            <span slot="footer" class="dialog-footer">
                <el-upload
                    ...
                    :headers="this.$getHeader()"
                    drag
                    :action="this.$http.defaults.baseURL + '/mdmdistribute/uploadIpa'"
                    name="ipa"
                    accept=".ipa"
                    :multiple="false">
                <i class="el-icon-upload"></i>
    ...
    <el-dialog
      ...
      <el-upload
          ...
          class="upload-demo"
          :action="this.$http.defaults.baseURL + '/mdmdistribute/uploadImg'"
          :file-list="fileList"
          limit="4"
          list-type="picture"
          :auto-upload="false">
...
export default {
  ...
  methods: {
    updateDomain(id){
      ...
      var that = this
      this.$http.post('/mdmdistribute/updateDomain','id=' + id)
    ...
    updateDownCodeUrlPost(){
      ...
      var that = this
      this.$http.post('/mdmdistribute/updateBuyDownCodeUrl?id=' + that.id3 + "&url=" + that.buyDownCodeUrl)
          .then(function (response) {
            that.$load(0)
            if(response.data.code == 0){
              that.update(that.page,that.count);
    ...
    use(scope){
      ...
      this.$http.post('/mdmdistribute/updateDownCodeStatus?id=' + scope.row.id + "&downCode=" + scope.row.downCode)
          .then(function (response) {
            that.$load(0)
            if(response.data.code == 0){
              that.update(that.page,that.count);
    ...
    updateIntroducePost(){
      this.dialogVisible2 = false
      var that = this
      this.$http.post('/mdmdistribute/updateIntroduce?id=' + that.id2 + "&introduce=" + that.introduce)
          .then(function (response) {
            that.$load(0)
            if(response.data.code == 0){
              that.update(that.page,that.count);
    ...
    updateLanguagePost(){
      ...
      var that = this
      this.$http.post('/mdmdistribute/updateLanguage?id=' + that.id2 + "&language=" + that.language)
          .then(function (response) {
            that.$load(0)
            if(response.data.code == 0){
              that.update(that.page,that.count);
    ...
    uploadImagesPost(){
      if(this.img.length !=4){
        ...
        var that = this
        this.$http.post('/mdmdistribute/uploadImg',fordata)
            .then(function (response) {
              that.$load(0)
              if(response.data.code == 0){
                that.update(that.page,that.count);
    ...
    handleClick(row) {
      var that = this
      this.$http.post('/mdmdistribute/deleIpa?id=' + row)
          .then(function (response) {
            that.$load(0)
            if(response.data.code == 0){
              that.update(that.page,that.count);
    ...
    update(pageNum,pageSize){
      var that = this
      that.$load(1,"查询中")
      this.$http.get('/mdmdistribute/queryAccountAll?pageNum=' + pageNum + '&pageSize=' + pageSize)
          .then(function (response) {
            that.$load(0)
            if(response.data.code == 0){
